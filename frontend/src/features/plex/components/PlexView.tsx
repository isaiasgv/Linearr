  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-slate-800 shrink-0">
        ...
        <p className="text-xs text-slate-500 mt-0.5">Server info, library stats, and recently added content</p>
      </div>
              ...
              <div>
                ...
                <p className="text-xs text-amber-300/60">
                  ...
                  {serverInfo.plex_pass && (
                    <span className="ml-2 bg-amber-600/40 text-amber-200 rounded-full px-2 py-0.5 text-xs">Plex Pass</span>
                  )}
                </p>
              </div>
            </div>
          ...
          {loadingStats ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {libraryStats.map((lib) => (
                <div key={lib.id} className="bg-slate-800 border border-slate-700 rounded-xl p-4 hover:border-slate-600 transition-colors">
                  <div className="flex items-center gap-2 mb-2">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                      lib.type === 'movie' ? 'bg-purple-900/50 text-purple-400' : 'bg-blue-900/50 text-blue-400'
                    }`}>
                      {lib.type === 'movie' ? (
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                          <rect x="2" y="7" width="20" height="15" rx="2" /><circle cx="12" cy="14" r="2" />
                        </svg>
                      ) : (
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                    ...
                    <span className={`text-xs rounded-full px-1.5 py-0.5 font-medium ${
                      lib.type === 'movie' ? 'bg-purple-900/40 text-purple-300' : 'bg-blue-900/40 text-blue-300'
                    }`}>
                      {lib.type === 'movie' ? 'Movies' : 'Shows'}
                    </span>
                  </div>
                  <p className="text-sm font-semibold text-slate-100 truncate">{lib.title}</p>
                  <p className="text-2xl font-bold text-white mt-1">{lib.total_items.toLocaleString()}</p>
            ...
            <div className="flex gap-3 overflow-x-auto pb-2" style={{ scrollbarWidth: 'thin' }}>
              {recentItems.map((item) => (
                <div key={item.rating_key} className="shrink-0 w-28">
                  <div className="relative rounded-lg overflow-hidden bg-slate-800 mb-1.5" style={{ aspectRatio: '2/3' }}>
                    {item.thumb ? (
                      <img
                      ...
                      <div className="absolute inset-0 flex items-center justify-center text-slate-600">
                        <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1}>
                          <rect x="2" y="7" width="20" height="15" rx="2" /><circle cx="12" cy="14" r="3" />
                        </svg>
                      </div>
                    )}
                    <span className={`absolute top-1 right-1 text-xs rounded px-1 py-0.5 font-medium shadow ${
                      item.type === 'movie' ? 'bg-purple-600/80 text-white' : 'bg-blue-600/80 text-white'
                    }`}>
